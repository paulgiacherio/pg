...
This file layers on top of the root `CLAUDE.md` at `pg—live/CLAUDE.md`. Read both before working on this site.
...
**`--row-unit` rules:**
- The clamp bounds (`60px` min, `100px` max) are intentional — at extreme aspect ratios the 12-row fill is deliberately relaxed in favor of layout sanity. Adjust the bounds if the design evolves, but keep clamp in place.
...
- **Prefer keeping `--row-unit` as a `clamp()`** — removing the bounds risks runaway row sizes at extreme aspect ratios; tightening them too aggressively breaks the viewport-fill at normal sizes.
